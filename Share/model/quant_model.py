...
from .layers import SeNetLayer, DnnLayer, CINLayer
...
class QuantModel(tf.keras.Model):
    def __init__(self, config, **kwargs):
        ...
        self.cin_layer = CINLayer(
            cin_size=self.config.get("cin_size", [32, 32]),
            seed=self.config.get("seed", 1024),
        )
        ...
        self.cin_output_layer = tf.keras.layers.Dense(1, activation=None)
    ...
    def call(self, inputs, training=False):
        ...
        # # print(f"wide_output: {wide_output.get_shape()}")
        ...
        deep_input = self.senet_layer(deep_features)
        # 处理Deep侧DNN特征
        ...
        # 处理Deep侧CIN特征
        cin_output = self.cin_layer(deep_input)  # [B, cinSize]
        cin_logit = self.cin_output_layer(cin_output)

        # 最终输出
        # final_logit = wide_logit + dnn_logit + cin_logit
        return dnn_logit + cin_logit
