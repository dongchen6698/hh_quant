...
class UploadFromAkToDb:
    ...
    def upload_stock_individual_info(self):
        """
        个股信息数据
        """
        stock_info_list = []
        ...
        upload_df.to_sql(db_table_name, self.db_conn, if_exists="replace", index=False)

    def upload_stock_history_info(self, start_date, end_date):
        """
        历史行情数据
        """
        stock_info_list = []
        ...
        upload_df.to_sql(db_table_name, self.db_conn, if_exists="append", index=False)

    def upload_stock_technical_analysis_info_daily(self, start_date, end_date):
        """
        基于历史行情数据的TA指标数据
        """
        stock_info_list = []
        for stock_code, stock_name in tqdm(self.get_stock_info_records(), desc="upload_stock_technical_analysis_info_daily..."):
            stock_info = ak.stock_zh_a_hist(symbol=stock_code, adjust="hfq", start_date=start_date, end_date=end_date).iloc[:, :6]
            if not stock_info.empty:
                ...
                stock_info.ta.strategy("All", timed=True)
                stock_info.insert(0, "股票简称", stock_name)
                stock_info.insert(0, "股票代码", stock_code)
    ...
    def upload_stock_label_info_daily(self, start_date, end_date):
        stock_info_list = []
...
if __name__ == "__main__":
    ...
    # uploader.upload_stock_individual_info()
    # uploader.upload_stock_history_info(start_date, end_date)
    # uploader.upload_stock_technical_analysis_info_daily(start_date, end_date)
    uploader.upload_stock_label_info_daily(start_date, end_date)
